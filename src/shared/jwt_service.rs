    ...
    async fn from_request(request: &'r Request<'_>) -> request::Outcome<Self, Self::Error> {
        ...
        let auth_header = match request.headers().get_one("Authorization") {
            ...
            None => {
                return Outcome::Failure((
                    Status::Unauthorized,
                    ApiErrors::Unauthorized("Authorization Header Required".to_string()),
                ))
            }
        };
        ...
        let token = match get_token_from_header(auth_header) {
            ...
            None => {
                return Outcome::Failure((
                    Status::Unauthorized,
                    ApiErrors::Unauthorized("Authorization Header Malformed".to_string()),
                ))
            }
        };
        ...
        let id = match verify_token(token) {
            ...
            Err(_) => {
                return Outcome::Failure((
                    Status::Unauthorized,
                    ApiErrors::Unauthorized("Failed to verify token".to_string()),
                ))
            }
        };

        // Return the token
        Outcome::Success(Token { id })
    }
}
