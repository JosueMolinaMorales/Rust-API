...
pub async fn update_user(
    ...
    if updated_user.email.is_none() && updated_user.password.is_none() {
        return Err(ApiErrors::BadRequest("Email or Password is required".to_string()))
    }

    // Hash password
    if let Some(password) = updated_user.password {
        updated_user.password = Some(bcrypt::hash(password).map_err(|err| ApiErrors::ServerError(err.to_string()))?)
    }
